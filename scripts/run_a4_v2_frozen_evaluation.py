"""Resumable local frozen evaluation for the verified A4_v2 checkpoints.
...
CONTRACT = ROOT / "contracts" / "A4_v2_eval_contract.yaml"
TRAIN_RUN = ROOT / "runs" / "A4_v2_fresh_base_200"
...
def preflight(contract: dict) -> dict:
    ...
    for checkpoint in CHECKPOINTS:
        ...
        checks.append({"name": f"{checkpoint}/adapter_config", "expected": "A4 decoder Q/V r16 alpha32", "actual": "valid" if valid_adapter else adapter})
...
def main() -> int:
    ...
    if result["status"] != "PASSED":
        save_json(RUN / "evaluation_progress.json", {"status": "BLOCKED_A4_V2_FROZEN_EVALUATION", "preflight": result["status"]})
        return 2
    import torch
    ...
    save_json(state_root / "a4_v2_frozen_eval_state.json", {"status": "RUNNING", "planned_targets": 28, "current_target": {"checkpoint": CHECKPOINTS[0], "dataset": targets[0][0]}})
    save_json(state_root / "a4_v2_frozen_eval_heartbeat.json", {"status": "RUNNING", "pid": os.getpid()})
    save_json(state_root / "a4_v2_frozen_eval_progress.json", {"planned_targets": 28, "completed_targets": 0})
